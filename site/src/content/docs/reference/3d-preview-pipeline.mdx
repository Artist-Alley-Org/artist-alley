...
The script borrows the iterative-NDC-fit pattern from the
`thumbnails_renderer` codebase rather than the naive
`distance = maxDim * 2.2` heuristic:
...
through the turntable arc. This avoids a subtle Euler-rotation bug
that left the FOV-math first pass with empty frames.
