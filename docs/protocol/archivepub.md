...
| `encryption_required` | **Receiver-side variant** (ACTIVE at v0.8 / 1.22.I-h): plaintext envelope arrived for a target whose share sensitivity tier mandates encryption — the sender violated the v0.7 §3.6 policy. Distinct from `encryption_required_but_not_supported`, which is the recipient declaring inability to receive plaintext. Defense in depth: I-g's sender-side refusal is the primary enforcement; this gate refuses what a misbehaving sender should never have sent. Reference implementation activation is gated on `SensitivityLookup` being wired at boot (deferred until sensitivity columns ship on `posts` / `assets` — pre-MVP gap documented in `outbox/resolver.go`); the gate's wiring + reject reason are present in v0.8 so future activation is one boot-wire edit, not a wire-format change. |
