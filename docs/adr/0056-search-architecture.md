...
- Follow-up issues: #183 (CLIP visual encoder — SHIPPED PR #199), #184 (feedback loop — SHIPPED PR #208), #185 (visibility retrofit — SHIPPED PR #213, scope-trimmed per pre-audit; see §4 for the three deferred sub-scopes), #186 (AdminJobBackfillPage), #209 (split `search.Counter` latency window from per-result-class request counter — cheap refactor), #210 (sensitivity-column semantics for `visibility.Filter(EntityAsset)` — unify by-image coarse floor, deferred from #185 pre-audit), #211 (`FieldVisibility` API for IIIF metadata gating — deferred from #185 pre-audit), #212 (sqlc migration path for list-handler visibility consolidation — deferred from #185 pre-audit), #214 (MDX braced-identifier CI gate on docs PRs — deferred docs-tooling from PR #213)
