...
- `POST /search/by-image` reserved 501 in B-3 pending CLIP visual encoder sidecar (follow-up #183). Existing CLIP provider is Ollama nomic-embed-text (text-only). `Query.SimilarityHintID = "image:<sha256>"` when the endpoint activates.
