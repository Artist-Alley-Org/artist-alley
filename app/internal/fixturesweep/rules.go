...
var Rules = []Rule{
	...
	},
	{
		...
		IDColumn: "id",
		Kind:     "post",
		// Posts have no marker and no usable date boundary, so the rule
		// names the fixture families explicitly. Anything unrecognised
		// falls in the unclassified gap and is left alone.
		Fixture: `(title IS NULL OR title = '' OR ` +
			`title ~ '^(share|acl|ui|UI-)[0-9]*[ _-]' OR ` +
			`title ~ '[0-9]{13}' OR ` +
			`title ~* '(dogfood|fixture|probe)')`,
		Protected: `author_user_ref <> 1 OR created_at < '2026-08-17'::timestamptz`,
		Why: "Fixture posts are all authored by the bootstrap admin (ref 1) after the " +
			"seed, and all carry a dogfood/fixture/probe marker, a 13-digit epoch-ms " +
			"suffix, or a blank title. Verified 2026-08-20: 430 matched, 847 real " +
			"(32 distinct authors), and the admin's 9 genuine seeded posts predate the " +
			"seed boundary so Protected keeps them.",
	},
	{
