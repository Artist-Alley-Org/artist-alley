-- artist-alley baseline migration 00001 — DB schema baseline (pre-MVP).
--
-- This file SUPERSEDES the prior 00001..00056 migration sequence.
-- Per ADR 0046 (migration baseline + squash policy), we flatten
-- once before MVP so subsequent phases build on a clean slate.
-- There is NO upgrade path from the old sequence; dev installs
-- MUST `docker compose down -v` after pulling.
--
-- Source: pg_dump --schema-only of a fresh DB with every prior
-- migration applied, then hand-edited per the structural cleanup
-- decisions captured in docs/cleanup/audit-report.md. Two
-- categories of edit landed:
--
--   1. Dropped 15 unused columns from the `assets` table (legacy
--      import scaffolding that no Go code referenced; reintroduce
--      via fresh additive migrations if/when needed).
--
--   2. Renamed legacy-prefixed user-FK columns to the canonical
--      `*_user_ref` shape so the schema reads consistently
--      everywhere (e.g. `user_ref`, `granted_by_user_ref`,
--      `actor_user_ref`).
--
-- Pre-MVP everything is volatile (see feedback_pre_mvp_everything_is
-- _volatile.md). The baseline is the source of truth from here on;
-- everything that follows is incremental.
