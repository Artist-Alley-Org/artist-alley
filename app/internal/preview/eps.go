...
func (h *EPSHandler) fanPosterToLadder(ctx context.Context, hash, posterPath string) error {
	...
	for _, v := range cfg.Variants {
		...
		if err != nil {
			h.Logger.LogAttrs(ctx, slog.LevelWarn, "preview.eps.encode_failed",
				slog.String("variant", v.Key),
				slog.String("err", err.Error()))
			continue
		}
		if _, err := h.Storage.Backend.Put(ctx, hash, v.Key, bytes.NewReader(buf.Bytes())); err != nil {
			return fmt.Errorf("backend put eps variant %s: %w", v.Key, err)
		}
