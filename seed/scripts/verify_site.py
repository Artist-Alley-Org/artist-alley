...
-------------------------------------------------------
  ...
  preservation      Before/after: every `*.bak`, `dataset-metadata.json`
                    and `kenney-hq-replacements.json` present at the site
                    is byte-equal to an explicit baseline (`--baseline`,
                    recorded before staging, or `--reference`, a
                    directory holding the pre-publish copies), and the
                    site's ATTRIBUTIONS.md equals the repository copy.
...
# the two named files live at the site root.
